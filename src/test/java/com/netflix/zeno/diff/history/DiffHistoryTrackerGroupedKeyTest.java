...
public class DiffHistoryTrackerGroupedKeyTest extends DiffHistoryAbstractTest {
    ...
    public void testHistory() throws IOException {
        ...
        assertDiffObjectHistoricalState(object1History, arr(2, 3, 4), arr(2, 3), arr(3), arr(1, 2, 3));
        assertDiffObjectHistoricalState(object2History, arr(1), arr(1), arr(1, 2), null);
        assertDiffObjectHistoricalState(object3History, null, arr(4), arr(4), arr(4));
    }
    ...
    private void assertDiffObjectHistoricalState(List<DiffObjectHistoricalTransition<List<TypeA>>> list, int[]... expectedHistory) {
        for(int i=0;i<list.size();i++) {
            int[] expectedFrom = expectedHistory[i];
            int[] expectedTo = expectedHistory[i+1];
